...
export default function EventCard({ id, eventName, location, startTime, status, imageUrl, createdAt }: EventCardProps) {
    ...
    return (
        <Link href={`/events/${id}`} className="block group h-full">
            <article className="bg-white rounded-[1.5rem] shadow-[0_2px_8px_rgba(0,0,0,0.04)] hover:shadow-[0_12px_24px_rgba(0,0,0,0.08)] border border-gray-100 overflow-hidden transition-all duration-300 h-full flex flex-col group-hover:-translate-y-1">
                <div className="relative aspect-[4/3] w-full overflow-hidden bg-gray-100">
                    ...
                    <div className="absolute top-3 left-3">
                        <span className={`
                            ...
                            ${status === 'available'
                                ? 'bg-white/95 text-emerald-700 border-emerald-100/50'
                                : 'bg-gray-900/95 text-gray-200 border-gray-800'
                            }
                        `}>
                ...
                <div className="p-5 flex flex-col flex-1">
                    <div className="mb-2">
                        <h3 className="font-serif text-lg font-bold text-gray-900 leading-tight mb-2 group-hover:text-brand-orange transition-colors line-clamp-2">
                            {title}
                        </h3>
